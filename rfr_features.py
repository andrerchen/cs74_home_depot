...
if REBUILD:
    df_sims_train = pd.read_csv('data/train_run_v2.csv')            # UPDATE THIS
    df_sims_test = pd.read_csv('data/test_run_v2.csv')              # UPDATE THIS
...
# Google spell check is nice, but slow 
...
# build a spellcheck dictionary off of the results of Google spell check
...
if REBUILD:
    df_all['search_term'] = df_all['search_term'].map(lambda x:dict_spell_check(x))
...
rfr = RandomForestRegressor(max_depth=None, max_features="sqrt", n_estimators = 1500, n_jobs = 1, random_state = 2016, verbose = 1)
...
clf = pipeline.Pipeline([
        ...
        ('rfr', rfr)])
param_grid = {'rfr__n_estimators': [500]}
...
print(model.best_score_ + 0.47057)

y_pred = model.predict(X_test)
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('submission_rfr_no_brand_feature.csv',index=False)
print("--- Training & Testing: %s minutes ---" % round(((time.time() - start_time)/60),2))

# from sklearn.feature_extraction import text
# import nltk

# df_outliers = pd.read_csv(DF_ALL, encoding="utf-8", index_col=0)
