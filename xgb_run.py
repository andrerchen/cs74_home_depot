...
# also need to import model selection from sklearn
...
xgb_model = xgb.XGBRegressor(learning_rate=0.25, silent=True, objective="reg:linear", nthread=8, gamma=0.75, min_child_weight=4, max_delta_step=0,
                 subsample=0.55, colsample_bytree=0.7, colsample_bylevel=1, reg_alpha=0, reg_lambda=1, scale_pos_weight=1,
                 base_score=0.5, seed=10, missing=None)

# xgb_model = xgb.train(silent=False, objective="multi:softmax", nthread=-1, gamma=0, min_child_weight=1, max_delta_step=0,
#                  subsample=1, colsample_bytree=1, colsample_bylevel=1, reg_alpha=0, reg_lambda=1, scale_pos_weight=1,
#                  base_score=0.5, seed=0, missing=None)
...
clf = pipeline.Pipeline([
        ...
        ('xgb_model', xgb_model)])
param_grid = {'xgb_model__max_depth': [9], 'xgb_model__n_estimators': [150], 'xgb_model__min_child_weight': [8], 'xgb_model__learning_rate': [0.1], 'xgb_model__gamma': [3]}
...
pd.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('submission_xgb_v1.csv',index=False)
